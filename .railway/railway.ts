...
export default defineRailway(() => {
  ...
  const marketAiOrc = service("market-ai-orc", {
    ...
    replicas: { "sfo": 1 },
    deploy: { restartPolicyType: "ALWAYS" },
    env: { AI_MAX_TOOL_ITERATIONS: preserve(), AI_MODEL: preserve(), AI_REASONING_EFFORT: preserve(), CATALOG_DATABASE_URL: preserve(), MARKET_AI_ORC_API_KEY: preserve(), MARKET_AI_ORC_DB_PASSWORD: preserve(), OPENROUTER_API_KEY: preserve(), PORT: preserve() },
  });
  ...
  return project("lucid-patience", {
    resources: [marketAiBackend, marketAiOrc, idxPriceCron, telegramTrigger, marketAnalyticsWorker, DB2, telegramMonitor, idxPriceRecoveryCron, Postgres, marketQuerySandbox, aiDataCoverage, feature01Worker, dbOpsRunner, postgresVolumeThQL, postgresVolume, marketAnalyticsInput],
  });
});
