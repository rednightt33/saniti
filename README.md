...
The market-AI database foundation and Release 1B backend code are ready: generic tool, relationship, readiness, point-in-time/availability, analysis audit, version snapshot, and Golden Test contracts are registered, and deterministic acceptance is 15/15 PASS. The private Railway service is staged with least-privilege database access, approved limits, and its OpenAI secret; source deployment is the next active release gate. Database query, Analytics Worker, and LLM context/cumulative token limits are separate policies. Historical Feature 1–3 analysis uses conservative close-`t` to entry-`t+1` timing and must disclose survivorship/current-metadata limitations.
