...
19. [`apps/market-analytics-worker/README.md`](apps/market-analytics-worker/README.md) — isolated generic SQL analytics over immutable bounded Feature snapshots, without database credentials.
...
The market-AI database foundation, private backend, and generic Release 2 Analytics Worker are deployed. Every active Feature 1–3 column has a calculation-verified interpretation, recommended use, misuse warning, and evidence path. Data tools auto-load only relevant compact definitions; the full 91-row catalog is not injected. Simple episodes use `find_condition_runs`; joins, forward outcomes, windows, and niche computation use one `run_analytics_job` surface over immutable bounded Feature-only snapshots. The isolated worker has no database or bucket credentials. Runtime QC is conditional and scoped; warnings and valid anomalies continue, while impossible data blocks only the affected conclusion. Each session begins with a compact capability/limit/query-practice handoff, repeated discovery is bounded and cached, successful analytics creates evidence automatically, and the explicit completion gate stops further queries once necessary evidence is sufficient. Provider-call audit, independent database/worker/LLM limits, 64k hard active-context ceiling, 150k cumulative-input ceiling, and current-metadata/survivorship warnings remain enforced.
