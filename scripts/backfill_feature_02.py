#!/usr/bin/env python3
"""SQL-side, resumable Feature 02 backfill; never loads raw rows into Python RAM.

The temporary daily aggregate is rebuilt per run. Each ticker's final insert is
one transaction, so a failed ticker can be retried without partial Feature rows.
"""
...
CREATE_TEMP = """
CREATE TEMP TABLE feature_02_daily_base (
    date date NOT NULL,
...
STAGE_YEAR = """
INSERT INTO feature_02_daily_base (
    date, ticker, broker, market_board,
    buy_value, sell_value, buy_lots, sell_lots, net_value, net_lots
)
SELECT "Date", "Symbol"::text, "Broker"::text, "Market Board"::text,
...
INSERT_TICKER = """
INSERT INTO public."Feature_02_Broker_Rolling" (
    date, ticker, broker, market_board, broker_type, broker_classification,
...
WITH daily AS MATERIALIZED (
    SELECT * FROM feature_02_daily_base WHERE ticker = %s
),
calendar AS (
...
pairs AS (
    SELECT DISTINCT broker, market_board FROM daily
),
dense AS (
    SELECT calendar.date, calendar.day_no, pairs.broker, pairs.market_board,
...
rolling AS (
    ...
    FROM dense
    WINDOW w5 AS (PARTITION BY broker, market_board ORDER BY day_no
                  ROWS BETWEEN 4 PRECEDING AND CURRENT ROW),
           w20 AS (PARTITION BY broker, market_board ORDER BY day_no
                   ROWS BETWEEN 19 PRECEDING AND CURRENT ROW),
           w60 AS (PARTITION BY broker, market_board ORDER BY day_no
...
history AS (
    ...
    FROM complete
    WINDOW h AS (PARTITION BY broker, market_board ORDER BY day_no
                 ROWS BETWEEN 252 PRECEDING AND 1 PRECEDING)
)
SELECT daily.date, daily.ticker, daily.broker, daily.market_board,
       profile.broker_type, profile.broker_classification,
...
def connection_kwargs(args: argparse.Namespace) -> dict[str, object]:
    return {
        ...
        "sslmode": "require",
        "application_name": "feature-02-backfill",
        "autocommit": True,
    }
...
def main() -> None:
    ...
    parser.add_argument("--ticker", help="Only one source symbol; useful for a controlled test")
    parser.add_argument("--ticker-from", help="Inclusive ticker lower bound for a disjoint worker")
    parser.add_argument("--ticker-to", help="Exclusive ticker upper bound for a disjoint worker")
    parser.add_argument(
        "--rebuild-ticker", action="store_true",
        help="Replace the --ticker Feature rows transactionally after a source/profile change",
    )
    parser.add_argument("--max-tickers", type=int, help="Stop after this many new tickers")
    ...
    with psycopg.connect(**connection_kwargs(args)) as db:
        ...
        for year in range(bounds[0].year, bounds[1].year + 1):
            ...
            print(
                ...
                f"seconds={time.monotonic() - begun:.1f}",
                flush=True,
            )
        begun = time.monotonic()
        db.execute(
            "CREATE UNIQUE INDEX feature_02_daily_base_key "
            "ON feature_02_daily_base (ticker, broker, market_board, date)"
        )
        db.execute("ANALYZE feature_02_daily_base")
        staged = db.execute("SELECT count(*) FROM feature_02_daily_base").fetchone()[0]
        print(f"stage_ready rows={staged} index_seconds={time.monotonic()-begun:.1f}", flush=True)
        tickers = [row[0] for row in db.execute(
            "SELECT DISTINCT ticker FROM feature_02_daily_base ORDER BY ticker"
        )]
        completed = 0
        skipped = 0
        inserted_total = 0
        for ticker in tickers:
            ...
            with db.transaction():
                ...
                if not args.rebuild_ticker and db.execute(
                    'SELECT 1 FROM public."Feature_02_Broker_Rolling" '
                ...
                if args.rebuild_ticker:
                    db.execute(
                        'DELETE FROM public."Feature_02_Broker_Rolling" WHERE ticker = %s',
                        (ticker,),
                    )
                cursor = db.execute(INSERT_TICKER, (ticker,))
                expected = db.execute(
                    "SELECT count(*) FROM feature_02_daily_base WHERE ticker = %s",
                    (ticker,),
                ).fetchone()[0]
            ...
            if completed % 25 == 0 or elapsed >= 30 or args.ticker:
                print(
                    ...
                    f"ticker={ticker} rows={expected} inserted_total={inserted_total} "
                    f"ticker_seconds={elapsed:.1f}",
                    flush=True,
                )
        ...
        print(
            ...
            f"completed_tickers={completed} "
            f"skipped_tickers={skipped} total_tickers={len(tickers)}",
            flush=True,
        )
